...
* `admin_city.gpkg` delineates area of interest for the study (administrative boundary), optionally consists of smaller administrative units (e.g. districts) with a corresponding `pop` field to be used for population disaggregation. More details in [Population Disaggregation].
...
cities<-
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      population_data<-city$admin
      
      if("pop" %in% names(population_data) == F){
        city$grid <- city$grid%>% mutate(pop = NA)
      ...
      population_data%>%
        st_transform(crs = 4326)%>%
        select(pop)%>%
        mutate(pop = replace_na(pop,0))%>% # replace NA pop with 0
        mutate(pop_id = 1:length(pop)) #based on city district
...
cities<-
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      city_pop <- city$accessibility_min_tt$pop%>%sum(na.rm = T)
      ...
      spider_summary <- accessibility_min_tt%>%
        ...
        select(c("grid_id",classificationScheme$Class_C%>%unique,"meanMin_tt","pop"))%>%
        pivot_longer(cols = classificationScheme$Class_C%>%unique, names_to = "Class_C",values_to = "tt")%>%
        filter(!is.na(pop)) # remove missing pop
      
      meanMin_tt_popweighted<-
        spider_summary%>%summarise(meanMin_tt_popweighted = weighted.mean(meanMin_tt,pop))

      spider_summary <-
        spider_summary%>%
        group_by(Class_C)%>%
        summarise(tt_popweighted = weighted.mean(tt,pop))%>%
