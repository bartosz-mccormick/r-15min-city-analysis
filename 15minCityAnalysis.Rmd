---
title: "15 MC Analysis"
subtitle: "Berlin and Munich"
output: 
  html_notebook:
    code_folding: hide
    toc: yes
...
* `results` is an empty folder where results of the script will be saved
* `admin_city.gpkg` delineates area of interest for the study (administrative boundary), optionally consists of smaller administrative units (e.g. districts) with a corresponding `population` field to be used for population disaggregation. More details in [Population Disaggregation].
* `osm_study_city.osm.pbf` is an extract of raw OSM data, the basis for the R5R network and POIs. **To avoid edge effects the area must be at least as large as the study area**.
...
For this case study, a Germany-wide `.pbf` file was sourced from [Geofabrik](https://download.geofabrik.de/europe.html) on January 6, 2023. [Osmosis](https://wiki.openstreetmap.org/wiki/Osmosis) was used for cropping the file into smaller, study-area-specific `.pbf` files. This may not be necessary in all cases, but excessively large `.pbf` files will negatively impact performance, particularly R5R. 

### Osmosis Processing

Osmosis processing was done in two steps:

1. Crop the Germany-wide `.pbf` file to an area **larger** than the study area. In this case: the bounding box of the administrative boundary buffered by 5000 m
...
cities%>%
  lapply(function(city){
    ...
    list(step1 = admin_union%>%st_buffer(5000,joinStyle = "BEVEL")%>% # 4000 m = 15 min @ 16 km/hr
           st_transform(crs = 4326)%>%
           st_bbox(),
         step2 = admin_union%>%st_buffer(4000,joinStyle = "BEVEL")%>% # 4000 m = 15 min @ 16 km/hr
           st_transform(crs = 4326)%>%
           st_bbox())
...
names(osmosis_bbox)%>%
  ...
  lapply(function(cityName){
    names(osmosis_bbox[[cityName]])%>%
      set_names(.)%>%
      lapply(function(stepName){
        paste("osmosis --read-pbf germany-latest.osm.pbf --bounding-box",
              paste0("top=", osmosis_bbox[[cityName]][[stepName]]$ymax),
              paste0("left=", osmosis_bbox[[cityName]][[stepName]]$xmin),
              paste0("bottom=", osmosis_bbox[[cityName]][[stepName]]$ymin),
              paste0("right=", osmosis_bbox[[cityName]][[stepName]]$xmax),
              "--write-pbf",
              ifelse(stepName == "step1",
                     paste0("osm_study_buf_",cityName,".osm.pbf"),
                     paste0("osm_study_",cityName,".osm.pbf")),
              sep =" ")
  ...
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      st_make_grid(city$study%>%
        ...
        mutate(in_admin = (st_intersects(st_centroid(.)%>%
                                           st_transform(crs = st_crs(4326)),
                                         city$admin)%>%lengths)==1) # flag (T/F) if centroid of cell is within admin boundary
...
# https://stackoverflow.com/a/55961448
...
names(cities)%>%
  ...
  lapply(function(cityName){
    ...
    ## subset of POIs for entrance collection
    ...
    city_walk_net<-
      city_walk_net%>%
      mutate(intersects_poi = st_intersects(.,tempPoisEntrances))%>%
      rowwise()%>%
      mutate(l = length(intersects_poi))%>%
      ungroup()%>%
      filter(l>0)%>%
      select(intersects_poi)
    
    
    
    # split large POIs into segments, goal being smaller,simpler linestrings which are easier to index
    # tempPoisEntrances <-
    #   tempPoisEntrances%>%
    #   stdh_cast_substring()%>%
    #   st_cast("LINESTRING")
    ...
    ## process the street network in chunks (to avoid using too much ram)
    
    city_walk_net_index <- seq(1,nrow(city_walk_net))
    
    tempEntrances<-
    split(city_walk_net_index, ceiling(city_walk_net_index/1000))%>% # process in chunks of 1000 street network elements
      lapply(function(city_walk_net_index_set){
        city_walk_net[city_walk_net_index_set,]%>% # network elements in chunk
          st_intersection(tempPoisEntrances[.$intersects_poi%>%unlist()%>%unique,])%>% # perform intersection with POIs that have been already checked to intersect with the street network elements in the chunk 
          select(feature_id)
      })%>%
      bind_rows()
    ...
    tempEntrances<-
    tempEntrances%>%
      filter(st_geometry_type(.) %in% c("POINT","MULTIPOINT"))%>% # remove linestrings (e.g. border overlaps perfectly with street network)
      split(st_geometry_type(.) %in% c("MULTIPOINT"))%>%
      lapply(function(points){
        if(all(st_geometry_type(points)=="MULTIPOINT")){
          #cast multipoints to individual points
          points%>%st_cast("POINT")
        }else{
          points
        }
      })%>%
    ...
    # determine centroid for all other POIs
    tempCentroids <-
      tempPois%>%
      select(feature_id)%>%
    ...
    # tempCentroids <-
    # tempPois%>%
    #   split(st_geometry_type(.) == "MULTIPOLYGON")%>%
    #   lapply(function(pois){
    #     if(all(st_geometry_type(pois)!="MULTIPOLYGON")){
    #       #if not a multipolygon... centroid
    #       return(pois%>%st_centroid())
    #     }
    #     # if multipolgyon, cast to individual polygons and take centroids of those
    #     
    #     # NOT IMPLEMENTED - may be misleading. Stick to entrances only for this case
    #     
    #   })%>%
    #   bind_rows()
    ...
    #DEBUG
    # st_write(pois_access_points,"pois_debug.gpkg","access_points",append=FALSE)
    # st_write(tempPois,"pois_debug.gpkg","tempPois",append=FALSE)
    # st_write(city_walk_net%>%select(-intersects_poi),"pois_debug.gpkg","city_walk_net",append=FALSE)
    ...
    grid_centroids <- 
      ...
      # CONSIDER SNAP FEATURE
      # append nearest grid cell id
      pois_access_points <-
        pois_access_points%>%
        mutate(NearestGridCell = grid_centroids$grid_id[st_nearest_feature(.,grid_centroids)])%>%
        # remove multiple access points for a single grid cell
        st_drop_geometry()%>% # removing geometry necessary for distinct call to work
        distinct() 
...
cities<-
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      o_points <- 
        grid_centroids%>%
        filter(in_admin)   #origin only within admin boundary
...
Identifying residential buildlings enables population disaggregation. Areas are aggregated to the grid cells level
```{r results = "hide"}
  ...
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      st_read(gpkg_path, layer = "multipolygons")%>%
        ...
        mutate(area = st_area(.))%>%
        #fix geometries (again, after transform)
        st_make_valid()%>%
        filter(st_is_valid(.))%>% # if they can't be fixed, remove (negligible)
        filter(st_geometry_type(.) %in% c("POLYGON","MULTIPOLYGON"))
      ...
      potential_RB%>%
        ...
        select(building)%>%
        mutate(RB_id = 1:length(building))
      

      
      # Intersect RB with grid, assign each RB a vector of indices corresponding to the grid cells it intersects with
      
      potential_RB$grid_index <-
        st_intersects(potential_RB,grid)
      

      
      # potential RB that intersect multiple cells...
      
      multicell_potential_RB <-
        potential_RB%>%
        filter(lengths(grid_index) != 1)
      
      
      multicell_potential_RB_index <- seq(1,nrow(multicell_potential_RB))

      RB_area<-
      split(multicell_potential_RB_index, ceiling(multicell_potential_RB_index/5000))%>% # process in chunks of 5000 multicell RBs
        lapply(function(multicell_potential_RB_index_set){
          
          # indices of cells that intersect with RB in the chunk
          
          chunk_grid_indices_multi <- multicell_potential_RB$grid_index[multicell_potential_RB_index_set]%>%unlist()

          # grid cells relevant to chunk
          chunk_grid_multi <- grid[chunk_grid_indices_multi,]
          
          chunk_multicell_potential_RB <- multicell_potential_RB%>%select(-grid_index)%>%.[multicell_potential_RB_index_set,]
          
          chunk_multicell_potential_RB%>%
            st_intersection(chunk_grid_multi)%>%
            mutate(area = st_area(.))%>%
            st_drop_geometry()%>%
            group_by(grid_id)%>%
            summarise(area = sum(as.numeric(area,units = "m2")))
        })%>%
        bind_rows()%>%
        group_by(grid_id)%>%
      ...
      # potential RB that intersect a single cell...
      # are assigned the ID of the grid cell 
      ...
      RB_area<-
        bind_rows(RB_area,
                  potential_RB%>%
                    filter(lengths(grid_index) == 1)%>%
                    mutate(grid_id = grid$grid_id[unlist(grid_index)])%>%
                    select(-grid_index)%>%
                    mutate(area = st_area(.))%>%
                    st_drop_geometry()%>%
                    group_by(grid_id)%>%
                    summarise(area = sum(area))%>%
                    mutate(area = as.numeric(area,units = "m2")))%>%
        group_by(grid_id)%>%
        summarise(area = sum(area))
      
      
      city$grid <- city$grid%>%left_join(RB_area, by = "grid_id")
  ...
  names(cities)%>%
    ...
    lapply(function(cityName){
      ...
      if(length(gpkg_path) == 0){
        city$grid <- city$grid%>% mutate(population = NA)
...
cities%>%
  lapply(function(city){
    city$accessibility_summaries$spider_summary
  })%>%
  bind_rows()%>%
  arrange(desc(meanMin_tt_popweighted))
```
